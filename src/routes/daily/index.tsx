...
function AddTodo() {
  const { date, data: todos } = useLoaderData();
...
const TaskApp = () => {
  const { data: todos, date, streak_data } = useLoaderData();
  ...
  const calculateTotalTime = (
    ...
    return todos.reduce((total_time, todo) => {
      ...
      const sessionTime = todo.sessions.reduce(
        (sessionTotal, session) => sessionTotal + session.time,
        0,
      );
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <Popover>
            ...
            <PopoverContent className="w-auto">
              <CalendarWeek total_sessions={total_sessions} />
            </PopoverContent>
          </Popover>
          <Popover>
            ...
            <PopoverContent className="w-auto">
              <CalendarMonth total_sessions={total_sessions} />
            </PopoverContent>
          </Popover>
        ...
        >
          ...
          <div className="ml-auto flex items-center gap-2">
            <Button
              onClick={() =>
                submit({
                  day: date.timestamp - 86400000, // Mengurangi 1 hari (86400000 ms = 1 day)
                })
              }
              variant="outline"
            ...
            <Button
              onClick={() =>
                submit({
                  day: date.timestamp + 86400000, // menambah 1 hari (86400000 ms = 1 day)
                })
              }
              variant="outline"
              className="h-8 w-8 p-0"
      ...
      <div className="relative p-10 w-full border-4 border-black rounded-sm shadow-light h-full space-y-4 overflow-hidden">
        ...
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-2 pb-5">
          ...
          <div className="flex justify-center">
            <Link to={`/daily/add?day=${date.timestamp}`}>
  ...
  return (
    <div className="grid gap-3">
      {todos.map((todo, index) => {
        return <TodoForm key={todo.id} task={todo} />;
      })}
    </div>
  );
};
...
const TodoTimer = () => {
  ...
  const { date, data: todos, active_task } = useLoaderData();
  const submit = useSubmit();
  const routeAction = `/daily?day=${date.timestamp}`;
  let progress = 0;
      ...
      if (!timerRef.current) {
        timerRef.current = setInterval(() => {
          ...
          if (updatedTotalTime >= TWENTY_FIVE_MINUTES) {
            ...
            submit(
              {
                intent: "update-complete-task-session",
                id: active_task.id,
              ...
              <div className="flex flex-col justify-center">
                ...
                {(pauseTodo || active_task) && (
                  <Button
                    ...
                    variant="link"
                    style={{ animationDelay: `0.1s` }}
                    className={`transition-all duration-500 ease-in-out animate-roll-reveal [animation-fill-mode:backwards] z-10 ${active_task ? "text-orange-300" : "text-green-500"} `}
                  >
                    {active_task ? (
                      <Pause
          ...
          >
            ...
            <div className="mt-3 flex w-full flex-col gap-2 pr-14 md:flex-row md:items-center md:pr-0">
              ...
              <button
                ...
                icon="[object Object]"
                confirmationicon="[object Object]"
...
const CalendarMonth = () => {
  const { total_sessions } = useLoaderData();
...
const CalendarWeek = () => {
  const { total_sessions, streak_data } = useLoaderData();
  const [currentMonth, setCurrentMonth] = useState(new Date());
  const [daysInMonth, setDaysInMonth] = useState([]);
...
const TodoForm: React.FC = ({ task, index }) => {
  const todo = task;
  const fetcher = useDebounceFetcher();

  const fetcherDelete = useFetcher({ key: "delete" + todo.id });
  const fetcherPlay = useFetcher({ key: "play" + todo.id });
  const fetcherPause = useFetcher({ key: "play" + todo.id });
  const navigation = useNavigation();
  const submit = useSubmit();
  const { data: todos, date, active_task } = useLoaderData();
  const routeAction = `/daily?day=${date.timestamp}`;
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem asChild>
                        <button
                          className="w-full flex gap-x-1 cursor-pointer select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none transition-colors focus:bg-accent focus:text-accent-foreground"
                          onClick={() =>
                            submit(
                              {
                                intent: "add-sub-task",
...
const GardenSessionBadge = ({ sessionCount, text }) => {
  ...
  // Menentukan icon dan teks berdasarkan jumlah sesi
  if (sessionCount === 1) {
    icon = "🌱";
...
const DeleteTask: React.FC = ({ id, subtaskId, intent }) => {
  ...
  return (
    <button
      className="relative flex select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none transition-colors focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50 hover:bg-accent w-full"
      type="button"
      onClick={async () => {
        ...
        if (prompt) {
          submit(
            {
              intent: intent ? intent : "delete-task",
              id,
            },
            {
              action: routeAction,
