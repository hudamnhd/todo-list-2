...
export function DataTableToolbar<TData>({
  ...
  const filterRowsByStatus = (status: string) => {
    return rows.filter((row) => {
      ...
      return (
        created_at >= startOfDay(today) &&
        created_at <= endOfDay(today) &&
        taskStatus === status
      );
    });
  };
  ...
  const todayRows = rows.filter((row) => {
    const created_at = new Date(row.getValue("created_at"));
    return created_at >= startOfDay(today) && created_at <= endOfDay(today);
  });
  ...
  const totalTimeInMilliseconds = todayDoneRows.reduce((total, row) => {
    const created_at = new Date(row.getValue("created_at")).getTime();
    const completedAt = new Date(row.getValue("completedAt")).getTime();
    return total + (completedAt - created_at);
  }, 0);
